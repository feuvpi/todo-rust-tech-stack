...
impl TryForm<W<Value>> for Object {
    type Error = Error;
    fn try_from (val: W<Value>) -> Result<Object, Error> {
        match val.0 {
            Value::Object(obj) => Ok(obj),
            _ => Err(Error::XValueNotType("Object")),
        }
    }
}

impl TryForm<W<Value>> for Array {
    type Error = Error;
...
impl TryForm<W<Value>> for i64 {
    type Error = Error;
    fn try_from(val: W<Value>) -> Result<i64, Error> {
        match val.0 {
            Value::Array(obj) => Ok(obj.as_int()),
            _ => Err(Error::XValueNotOfType("i64")),
        }
    }
}

impl TryForm<W<Value>> for bool {
    type Error = Error;
...
impl TryForm<W<Value>> for String {
    type Error = Error;
